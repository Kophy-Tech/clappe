...
import { signIn, signOut, updUser, updGurd, signOutGurd, updStore, overwriteStore, storeError } from "./ActionCreators";
...
export const theStore = createReducer({ ...iState, error: false }, {
  [updStore]: (s, a) => {
    const newS = { ...current(s), error: false }
    if (a.payload) {
      const existStore = newS[a.payload.name]

      console.log("existing store ooo", existStore, "value oo", a.payload.value)
      ...
      if (existStore) {
        if (Array.isArray(existStore)) {
          ...
          console.warn("You are trying to merge an array with something not array and object")
          return ({
            ...newS,
            [a.payload.name]: [...existStore, a.payload.value]
          })
          ...
          if (Array.isArray(a.payload.value)) {
            console.warn("You are merging an object with array, it might destort your data to make keys as an integer")
          }
          return ({
            ...newS,
            [a.payload.name]: { ...existStore, ...a.payload.value }
        ...
        console.warn("There is a prob, existing store is not object and not array")
        return ({
          ...newS,
...
// export const guardian = createReducer({ ...iState, guardian: [] }, {
//   [updGurd]: (s, a) => {
//     if (a.payload) {
